  ...
  struct _csts_desc
{
  ...
  void *cmp_desc;
  int (*cmp) (void *, harglst *, hargkey_t * left_key, hargtype_t left_type,
...
static void
say_creating (void *list, unsigned size)
{
...
static void
say_closing (void *list, unsigned flag)
{
...
#ifdef HARG_DEBUG
static void
message (const char *s, const char *f, long u, long v, const char *t)
{
  fputs (s, stderr);
...
static void
debug (const char *s, long u, long v)
{
  message ("DEBUG: ", s, u, v, ".");
}
#endif /* HARG_DEBUG */
...
static harg *
create_harg (hargtype_t type, void *data, unsigned size)
{
  harg *h;
...
static void
clean_up (harglst * a, harg * data, hargkey_t * key, unsigned len)
{
  ...
  if (is_harglst_type (data->type) && a != 0 && (a->destroy_mode & H_sRECURSE))
...
static int
do_for_all_cb (do_for_all_cb_state * s, harg * h, hargkey_t * key, unsigned len)
{
  return (s->cb_fn) (s->state,
...
static harg *
a_copy (copy_cb * desc, harg * data,    /* from the source list */
        hargkey_t * key, unsigned len)
{
  unsigned size;
  harglst *a, *b, *c;

  if (data == 0)
    {                           /* may happen, bootstrap also */
      errno = 0;
      return 0;
    }
  ...
  if (is_harglst_type (data->type) && (a = data->d.d.ptr[0]) != 0)
    {
      ...
      if ((b->x =
           copy_hlst (a->x, query_hlst_size (a->x),
                      (void *(*)(void *, void *, char *, unsigned)) a_copy,
                      desc,
                      (void (*)(void *, void *, char *, unsigned)) clean_up,
                      0)) == 0)
        {
          int e = errno;
          XFREE (b);
...
static int __csts_cb            /* custum sort call back function descriptor */
  (void *desc, const char *left_key, unsigned left_klen, const char *right_key,
   unsigned right_klen)
{
  harglst *a = desc;
  ...
  return (*s->cmp) (s->cmp_desc, a, left_key, get_local_type ((*left_R)->type),
...
static void
do_printf (const char *f, harg ** R, void *a, int flags, int ptype,
           unsigned arg)
{
  if (R != 0)
    {
...
static void
do_harg_dump (harglst * a, int level)
{
  hargwalk *w;
  harg **R, *r;
...
static harg *
get_harg_entry (harglst * a, const char *key, unsigned len)
{
  harg **R, *r;
...
void
harg_close_any (harglst * a, int flag)
{
  if (a == 0)
    return;
  a->destroy_mode = flag;
...
harglst *
harg_dup (harglst * a, unsigned size)
{
  copy_cb desc;
  harg data;
...
hargkey_t *
harg_addt (harglst * a, hargkey_t * key, hargtype_t type, int overwrite,
...
int
harg_set_valuet (harglst * a, hargkey_t * key, hargtype_t type, unsigned size,
                 void *value)
{
  harg **R, *r;
  int klen = klen_by_type (type);
...
int
harg_renamet (harglst * a, hargkey_t * key, hargtype_t type, hargkey_t * nkey,
              hargtype_t ntype)
{
  harg **S, **R, *r;
  ...
  same_keys = nkey == 0 || (is_ptrkey_type (type) && is_ptrkey_type (ntype)
                            && memcmp (key, nkey, sizeof (void *)) == 0)
    || (is_ptrkey_type (type) == 0 && is_ptrkey_type (ntype) == 0
        && strcmp (key, nkey) == 0) ? 1 : 0;
...
int
harg_removet (harglst * a, hargkey_t * key, hargtype_t type)
{
  int klen = klen_by_type (type);
  harg **R = 0;
...
harg_get_valuet (harglst * a, hargkey_t * key, hargtype_t type)
{
  harg *r;
...
int
harg_inct (harglst * a, hargkey_t * key, hargtype_t type, incmode_t incop,
           int inc)
{
  harg *r, **R;
  int klen = klen_by_type (type);
  ...
  /* decrement */
  if (inc_op_notnegtv_record (incop)
      && (GPOINTER_TO_SIZE (r->d.d.ptr[0]) < inc))
    {
      errno = ERANGE;
      return -1;
    }
  if (inc_op_notpostv_record (incop)
      && (GPOINTER_TO_SIZE (r->d.d.ptr[0]) > inc))
    {
      errno = ERANGE;
      return -1;
    }
  if (inc_op_destroy0_record (incop)
      && (GPOINTER_TO_SIZE (r->d.d.ptr[0]) <= inc))
    {
      delete_hlst (a->x, key, klen);
      return errno = 0;
...
int
harg_csort (harglst * a,
            int (*fn) (void *, harglst *, hargkey_t * left_key,
                       hargtype_t left_type, hargkey_t * right_key,
                       hargtype_t right_type), void *fn_desc)
{
  csts_desc *s;
...
hargkey_t *
harg_get_ntht (harglst * a, unsigned inx, hargtype_t type)
{
  harg **R;
  if (a == 0)
    {
...
hargtype_t
harg_get_typet (harglst * a, hargkey_t * key, hargtype_t type)
{
  harg *r = get_harg_entry (a, key, klen_by_type (type));
  return r == 0 ? 0 : get_local_type (r->type);
}

unsigned
harg_get_sizet (harglst * a, hargkey_t * key, hargtype_t type)
{
  harg *r = get_harg_entry (a, key, klen_by_type (type));
  return r ==
    0 ? -1 : (r->type & RHARG_ANY & RHARG_PANY) ? sizeof (void *) : r->size;
}
...
hargkey_t *
harg_walk_nextT (hargwalk * w, hargtype_t * Type)
{
  harg **P;
         ...
         void *state)
{
  ...
  return for_hlst_do (a->x,
                      (int (*)(void *, void *, char *, unsigned)) do_for_all_cb,
                      &s);
}
...
void                            /* define a logger function */
  (*harg_logger (void (*f) (const char *, ...))) (const char *, ...)
{
  void (*g) (const char *, ...);
  g = xlog;
  xlog = f;
