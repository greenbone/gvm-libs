...
static unsigned size_hint_percentage_compressor = DEFAULT_PERCENTAGE_COMPRESSOR;

/* used for custum sorting -- not thread safe */
static void *sorter_desc;
static int (*sorter_fn) (void *, const char *, unsigned, const char *,
                         unsigned);
...
static hashqueue **
find_bucket_ptr (hashqueue ** Q, const char *key, unsigned len)
{
  hashqueue *q;
  while (q = *Q, q != 0)
    {
...
static int
__comp (hashqueue ** left, hashqueue ** right)
{
  int n, min;
...
static int
__comp_custom (hashqueue ** left, hashqueue ** right)
{
  return (*sorter_fn) (sorter_desc, (*left)->key, (*left)->keylen,
...
hlst *
create_hlst (unsigned estimated_size_hint,
             void (*clup) (void *, void *, char *, unsigned), void *state)
{
...
hlst *
copy_hlst (hlst * h, unsigned estimated_size_hint,
           void *(*copy) (void *, void *, char *, unsigned), void *cpstate,
           void (*clup) (void *, void *, char *, unsigned), void *state)
{
  const hash_defs *hd = hints;
  hlst *new;
...
void
flush_hlst (hlst * h, void (*clup) (void *desc, void *, char *, unsigned),
            void *desc)
{
  unsigned i;
  hsrch *s;
...
void **
find_hlst (hlst * h, const char *key, unsigned len)
{
  hashqueue **Q;
  int inx;
...
void **
make_hlst (hlst * _h, const char *key, unsigned len)
{
  hlst *h = (hlst *) _h;
  hashqueue *q;
  int inx;
...
int
delete_hlst (hlst * h, const char *key, unsigned len)
{
  hashqueue *q, **Q;
  hsrch *s;
  unsigned inx;
...
/* global factor to be applied internally to any estimated_size_hint */
unsigned
compress_hlst_index (unsigned size_hint_percentage)
{
...
close_hlst_search (hsrch * s)
{
  ...
  while (u = *U, u != 0)
    {
      if (u == s)
        {                       /* find the link pointer for that record */
      ...
      if (u->next == u)
        {                       /* XXXXXXXXXXXXX Grrr - should not happen (jh) */
          fprintf (stderr,
...
int
for_hlst_do (hlst * h, int (*fn) (void *, void *, char *, unsigned),
             void *state)
{
  unsigned i;
  int n;
...
sort_hlst (hlst * h)
{
  ...
  h->access =
    XMALLOC (sizeof (sorter) + (h->total_entries - 1) * sizeof (hashqueue *));
...
static void
__hstatistics_fn (hstatistics * state, unsigned fill)
{
  if (fill == 0)
    {
      state->buckets.idle++;
...
void
hlst_statistics (hlst * h, void (*fn) (void *, unsigned), void *state)
{
  unsigned i, sum;
  float var, mu;
  hstatistics hs;
