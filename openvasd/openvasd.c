                            ...
                            const void *val)
{
  ...
  switch (opt)
    ...
    case OPENVASD_SCAN_ID:
      (*conn)->scan_id = g_strdup ((char *) val);
      break;
    case OPENVASD_PORT:
    default:
      (*conn)->port = *((int *) val);
                       ...
                       char *path, char *data)
{
  ...
  if ((*conn)->port > 0 && (*conn)->port < 65535)
    {
      char buf[6];
      snprintf (buf, sizeof (buf), ":%d", (*conn)->port);
  ...
  if ((*conn)->ca_cert != NULL)
    {
      ...
      if (curl_easy_setopt (curl, CURLOPT_CAPATH, g_strdup ((*conn)->ca_cert))
          != CURLE_OK)
        {
  ...
  if ((*conn)->cert != NULL && (*conn)->key != NULL)
    {
      if (curl_easy_setopt (curl, CURLOPT_SSLCERT, (*conn)->cert) != CURLE_OK)
        {
      ...
      if (curl_easy_setopt (curl, CURLOPT_SSLKEY, (*conn)->key) != CURLE_OK)
        {
...
openvasd_resp_t
openvasd_get_scan_results (openvasd_connector_t *conn, unsigned long first,
                           unsigned long last)
{
  openvasd_resp_t response = NULL;
  GString *path = NULL;
  ...
  if ((*conn)->scan_id != NULL && (*conn)->scan_id[0] != '\0')
    {
      ...
      g_string_append (path, (*conn)->scan_id);
      if (last >= first)
        g_string_append_printf (path, "/results?range%lu-%lu", first, last);
      else
        g_string_append_printf (path, "/results");
    }
  else
    {
      response->code = RESP_CODE_ERR;
...
openvasd_get_scan_progress (openvasd_connector_t *conn)
{
  ...
  int running_hosts = 0, running_hosts_progress_sum = 0;
  ...
  running_hosts = json_reader_count_elements (reader);
  for (int i = 0; i < running_hosts; i++)
    {
      json_reader_read_element (reader, i);
...
add_vts_to_scan_json (gpointer single_vt, gpointer builder)
{
  ...
  builder = json_builder_add_string_value (builder, vt->vt_id);
  ...
  if (g_hash_table_size (vt->vt_values))
    {
      ...
      while (g_hash_table_iter_next (&vt_data_iter, (gpointer *) &vt_param_id,
                                     (gpointer *) &vt_param_value))
        {
          ...
          json_builder_set_member_name (builder, "id");
          builder = json_builder_add_int_value (builder, atoi (vt_param_id));
          json_builder_set_member_name (builder, "value");
          builder = json_builder_add_int_value (builder, atoi (vt_param_value));
          json_builder_end_object (builder); // end single param
        }
      json_builder_begin_array ((JsonBuilder *) builder); // End parameters list
    }
 ...
 */
openvasd_target_t *
openvasd_target_new (const char *hosts, const char *ports,
