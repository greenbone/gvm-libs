...
Ensure (xmlutils, parse_entity_parses_xml_with_attributes)
{
  ...
  const gchar *xml = "<a><b ba1='test'>1</b></a>";
...
Ensure (xmlutils, parse_entity_handles_declaration)
{
  ...
  const gchar *xml = "<?xml version=\"1.0\" encoding=\"UTF-8\"?><a><b ba1='test'>1</b></a>";
...
Ensure (xmlutils, parse_entity_handles_namespace)
{
  ...
  const gchar *xml = "<?xml version=\"1.0\" encoding=\"UTF-8\"?><a><n:b ba1='test'>1</n:b></a>";
...
Ensure (xmlutils, next_entities_handles_multiple_children)
{
  ...
  const gchar *xml = "<top><a>1</a><b></b><c>3</c></top>";
...
Ensure (xmlutils, parse_entity2_parses_simple_xml)
{
  xml_doc_t doc;
  entity2_t entity, b;
  ...
  assert_that (entity2_name (entity), is_equal_to_string ("a"));

  b = entity2_child (entity, "b");
  assert_that (entity2_name (b), is_equal_to_string ("b"));

  assert_that (entity2_text (doc, b), is_equal_to_string ("1"));
}

Ensure (xmlutils, parse_entity2_parses_xml_with_attributes)
{
  xml_doc_t doc;
  entity2_t entity, b;
  ...
  assert_that (entity2_attribute (b, "ba1"), is_equal_to_string ("test"));
}

Ensure (xmlutils, parse_entity2_handles_declaration)
{
  xml_doc_t doc;
  entity2_t entity, b;
...
Ensure (xmlutils, parse_entity2_handles_namespace)
{
  xml_doc_t doc;
  entity2_t entity, b;

  const gchar *xml = "<?xml version=\"1.0\" encoding=\"UTF-8\"?><a><n:b ba1='test' n2:ba2='test2'>1</n:b></a>";

  assert_that (parse_entity2 (xml, &doc, &entity), is_equal_to (0));

  assert_that (entity2_name (entity), is_equal_to_string ("a"));

  b = entity2_child (entity, "n:b");
  assert_that (entity2_name (b), is_equal_to_string ("n:b"));

  assert_that (entity2_text (doc, b), is_equal_to_string ("1"));

  assert_that (entity2_attribute (b, "n2:ba2"), is_equal_to_string ("test2"));
}

Ensure (xmlutils, parse_entity2_oval_timestamp)
{
  xml_doc_t doc;
...
"</oval_definitions>";
  ...
  assert_that (entity2_name (entity), is_equal_to_string ("oval_definitions"));
  generator_name = g_strdup ("generator");
  ...
  assert_that (timestamp, is_not_null);
  assert_that (entity2_text (doc, timestamp), is_equal_to_string ("2015-08-20T10:09:07.183-04:00"));
}

Ensure (xmlutils, parse_entity2_item_metadata)
{
  xml_doc_t doc;
  entity2_t entity, item, meta;

  const gchar *xml = "<cpe-list>" \
...
"</cpe-list>";
  ...
  item = entity2_child (entity, "cpe-item");
  assert_that (item, is_not_null);
  meta = entity2_child (item, "item-metadata");
  assert_that (meta, is_not_null);
  // NB
  assert_that (entity2_name (meta), is_equal_to_string ("item-metadata"));
  //assert_that (entity2_name (meta), is_equal_to_string ("meta:item-metadata"));
}
...
Ensure (xmlutils, next_entities2_handles_multiple_children)
{
  ...
  const gchar *xml = "<top><a>1</a><b>2</b><c>3</c></top>";

  assert_that (parse_entity2 (xml, &doc, &entity), is_equal_to (0));

  assert_that (entity2_name (entity), is_equal_to_string ("top"));

  children = entity2_children (entity);

  child = first_entity2 (children);
  assert_that (child, is_not_null);
  assert_that (entity2_name (child), is_equal_to_string ("a"));
  assert_that (entity2_text (doc, child), is_equal_to_string ("1"));
  children = next_entities2 (children);

  child = first_entity2 (children);
  assert_that (child, is_not_null);
  assert_that (entity2_name (child), is_equal_to_string ("b"));
  assert_that (entity2_text (doc, child), is_equal_to_string ("2"));
  children = next_entities2 (children);

  child = first_entity2 (children);
  assert_that (child, is_not_null);
  assert_that (entity2_name (child), is_equal_to_string ("c"));
  assert_that (entity2_text (doc, child), is_equal_to_string ("3"));
  children = next_entities2 (children);
}
...
main (int argc, char **argv)
{
  ...
  add_test_with_context (suite, xmlutils, parse_entity2_parses_simple_xml);
  add_test_with_context (suite, xmlutils, parse_entity2_parses_xml_with_attributes);
  add_test_with_context (suite, xmlutils, parse_entity2_handles_declaration);
  add_test_with_context (suite, xmlutils, parse_entity2_handles_namespace);
