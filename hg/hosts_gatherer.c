...
int
hg_next_host (struct hg_globals *globals, struct in6_addr *ip, char *hostname,
              int sz)
{
  struct hg_host *host;
  ...
  if ((globals->flags & HG_DNS_AXFR)
      && hg_filter_domain (globals, host->domain) == 0)
    {
            ...
            if (host->hostname && (!hg_valid_ip_addr (host->hostname)))
            ...
            else
              {
                ...
                if (IN6_IS_ADDR_V4MAPPED (ip))
                  inet_ntop (AF_INET,
                             (struct in_addr *) &host->in6addr.s6_addr32[3],
                             hostname, sz - 1);
                else
        ...
        if (IN6_IS_ADDR_V4MAPPED (&host->in6addr))
          {
            ...
            host->in6addr.s6_addr32[3] =
              htonl (ntohl (host->in6addr.s6_addr32[3]) + 1);
          }
        return 0;
      }
  }
