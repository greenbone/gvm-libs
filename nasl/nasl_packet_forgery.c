...

{
  ...
  if(!strcmp(element, "ip_src"))     {	snprintf(ret_ascii, sizeof(ret_ascii), "%s", inet_ntoa(ip->ip_src)); flag ++;} /* RATS: ignore */
  else if(!strcmp(element, "ip_dst")){  snprintf(ret_ascii, sizeof(ret_ascii), "%s", inet_ntoa(ip->ip_dst)); flag ++;} /* RATS: ignore */
...
#define rnd_tcp_port() (rand() % 65535 + 1024)
  ...
  if(islocalhost(&inaddr) > 0 )src.s_addr = dst->s6_addr32[3];
  else
  {
  ...
  else
  {
    for(i = 0; i < sizeof(sports) / sizeof(int) && ! flag; i ++)
    {
      ...
      ip->ip_src = src;  ip->ip_dst = inaddr;
