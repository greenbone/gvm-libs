...
tree_cell * nasl_start_denial(lex_ctxt * lexic)
{
 ...
 if(port)
 {
  ...
  if(soc>=0)
  {
   if(arg_get_value(script_infos, "denial_port") != 0)
    arg_set_value(script_infos, "denial_port", sizeof(gpointer), GSIZE_TO_POINTER(port));
   else
    arg_add_value(script_infos, "denial_port", ARG_INT, sizeof(gpointer), GSIZE_TO_POINTER(port));
 ...
 if(arg_get_value(script_infos, "tcp_ping_result") != 0)
  arg_set_value(script_infos, "tcp_ping_result", sizeof(gpointer), GSIZE_TO_POINTER(alive));
 else
  arg_add_value(script_infos, "tcp_ping_result", ARG_INT, sizeof(gpointer), GSIZE_TO_POINTER(alive));
...
{
 int port = GPOINTER_TO_SIZE(arg_get_value(lexic->script_infos, "denial_port"));
 int soc;
 int to = lexic->recv_timeout;
 struct arglist * script_infos = lexic->script_infos;
 tree_cell * retc = NULL;
 ...
 if(!port)
 {
  int ping = GPOINTER_TO_SIZE(arg_get_value(script_infos, "tcp_ping_result"));
...
nasl_open_sock_kdc(lex_ctxt* lexic)
{
  ...
  port = GPOINTER_TO_SIZE(plug_get_key(script_infos, "Secret/kdc_port", &type));
  if ( port <= 0 || type != KB_TYPE_INT )
	return NULL;


  tcp = GPOINTER_TO_SIZE(plug_get_key(script_infos, "Secret/kdc_use_tcp", &type));
