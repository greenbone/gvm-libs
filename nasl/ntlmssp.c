...
{
  SMBNTLMv2encrypt_hash(user, domain, ntlmv2_hash, challenge_data, address_list, address_list_len, lm_response, nt_response, session_key);
}
...
void ntlmssp_genauth_ntlm2(char *password, uint8_t *lm_response, uint8_t *nt_response, uint8_t *session_key, char *challenge_data, unsigned char* nt_hash)
{
  ...
  generate_random_buffer(lm_response, 8);
...
void ntlmssp_genauth_ntlm(char *password, uint8_t *lm_response, uint8_t *nt_response, uint8_t *session_key, char *challenge_data, unsigned char* nt_hash, int neg_flags)
{
  ...
  SMBencrypt_hash(lm_hash, (const uchar*)challenge_data, lm_response);
  SMBNTencrypt_hash(nt_hash, (uchar*)challenge_data, nt_response);

  if (neg_flags & NTLMSSP_NEGOTIATE_LM_KEY) {
    SMBsesskeygen_lm_sess_key(lm_hash, lm_response, session_key);
  } else {
